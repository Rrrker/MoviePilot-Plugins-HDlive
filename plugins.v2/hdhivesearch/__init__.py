...
class HDHiveSearch(_PluginBase):
    ...
    def _handle_search(self, channel, userid, keyword: str):
        ...
        try:
            ...
            if not tmdb_id:
                self._send_message(channel, userid, "搜索失败", f"未找到影片「{keyword}」的TMDB信息，请确认影片名称是否正确。")
                return
            
            resources = self._api.get_resources(media_type, tmdb_id)
            if not resources:
                self._send_message(channel, userid, "搜索结果", f"影片「{keyword}」暂无可用资源。")
                return
